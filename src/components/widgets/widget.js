...
class Widget extends React.Component {
  constructor(props) {
    ...
    nextWidgetId++; // currently not using, but may eventually
  ...
  handleOnMouseUpAnywhere = (event) => {
    ...
    // If we stopped dragging the mouse on a tile, i.e. if the drag was successful
  ...
  handleOnMouseMoveAnywhere = (event) => {
    ...
    if (event.target && event.target.id.startsWith("tile_")) {
      ...
      // Call TilesGrid so that all the tiles will be redrawn
  ...
  getTileWidth = () => {
    ...
    const resizedWidth = (this.state.resizeSrcX !== null)
      ? this.state.resizeDestX - this.state.resizeSrcX // width
      : null;
  ...
  getTileHeight = () => {
    ...
    const resizedHeight = (this.state.resizeSrcX !== null)
      ? this.state.resizeDestY - this.state.resizeSrcY // height
      : null;
